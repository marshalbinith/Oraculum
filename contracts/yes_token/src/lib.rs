#![no_std]
//! YES outcome token — SEP-41 token wrapping the shared [`token_base`] logic.
//! Minted to traders who buy YES; burned on sell, reward claim, or settlement.
...
impl YesToken {
    ...
    /// Mint tokens. Admin (Market) only.
    pub fn mint(env: Env, to: Address, amount: i128) {
        base::mint(&env, to, amount);
    }

    /// Burn tokens from a holder. Admin (Market) only.
    pub fn burn(env: Env, from: Address, amount: i128) {
        base::burn(&env, from, amount);
    }

    /// Allowance-based burn (spender authorizes).
